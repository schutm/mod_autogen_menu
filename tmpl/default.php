...
switch($mode) {
  case(12345678):
    ...
    *  The easiest way is to copy the code bellow and than just change it as you deserve
    *  Dont forget to change the number in CASE() to anything sane and then link it to output mode param in mod_autgen_menu.xml
    */
  break;
		...
		*/	
		if ($artfunction == 'links') {
			...
			foreach($list as $article) {
				$article_link = '<a href="'.$article->url.'" title="'.$article->title.'"><span>'.$article->title.'</span></a>';
				...
				if ($last_cat === false) {
					switch ($catfunction) {
						case 'links':
							echo '<ul class="menu'.$params->get('moduleclass_sfx').'"><li><a class="category" href="'.$category_link.'" title="'.$article->category.'">'.$article->category.'</a><ul><li>'.$article_link.'</li>';	
							break;
						case 'separators':
							echo '<ul class="menu'.$params->get('moduleclass_sfx').'"><li><span class="separator">'.$article->category.'</span><ul><li>'.$article_link.'</li>';					
							break;
						case 'hidden':
							echo '<ul class="menu'.$params->get('moduleclass_sfx').'"><li>'.$article_link.'</li>';
							break;
					}
				...
				/*  ------   N E W   C A T E G O R Y    ---- */	
					...
					switch ($catfunction) {
						case 'links':
							echo '</ul></li><li><a class="category" href="'.$category_link.'" title="'.$article->category.'">'.$article->category.'</a><ul><li>'.$article_link.'</li>';
							break;
						case 'separators':
							echo '</ul></li><li><span class="separator">'.$article->category.'</span><ul><li>'.$article_link.'</li>';
							break;
						case 'hidden':
							echo '<li>'.$article_link.'</li>';
							break;
